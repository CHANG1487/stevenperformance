...
專案使用以下環境變數（定義於 `.env`，不提交 git，參考 `.env.example`）：

| 變數名稱 | 說明 |
|---------|------|
| `VITE_GOOGLE_CLIENT_ID` | Google Cloud Console OAuth 2.0 用戶端 ID |
| `VITE_SPREADSHEET_ID` | Google Sheets 試算表 ID |
...
元件位於 `src/components/` 目錄下，依功能分類：

| 目錄 | 說明 |
|------|------|
| `src/components/auth/` | 登入、未授權頁 |
| `src/components/layout/` | Header、ProtectedRoute 等版面元件 |
| `src/components/frontend/` | 前台功能元件（問卷填寫相關） |
| `src/components/backend/` | 後台功能元件（管理、分析相關） |

新增元件步驟：
1. 在對應目錄建立 `ComponentName.jsx`
...
function MyComponent() {
  const { user, role, assignedSurveys, accessToken, logout } = useAuth()
  // ...
}
```

## Google Sheets API 呼叫方式

統一使用 `src/utils/sheetsApi.js` 提供的函式，避免直接在元件中 fetch：

```jsx
import { getSurveyList, appendSubmission } from '../../utils/sheetsApi'

// 讀取資料
const data = await getSurveyList(accessToken)

// 寫入資料
await appendSubmission(accessToken, rowData)
```

## 路由導向

使用 `react-router-dom` 的 `<Link>`、`useNavigate`，以及 `<ProtectedRoute minRole="主管">` 包裝受保護頁面：
